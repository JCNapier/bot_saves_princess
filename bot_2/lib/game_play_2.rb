...
class GamePlay 
  def play
    ...
    puts "Please enter a number greater than 1 and less than 100 to determine the grid size"
    size = gets.chomp.to_i
    until (size.positive?) && (size >= 2 && size < 100) do 
    ...
    range = Range.new(2, size)
    bot_row = gets.chomp.to_i 
    until (range.include?(bot_row)) do 
      puts "Not a valid selection. Please try again"
      bot_row = gets.chomp.to_i
    end 
    ...
    until (range.include?(bot_column)) do 
      puts "Not a valid selection. Please try again"
      bot_column = gets.chomp.to_i
    end
    ...
    until (range.include?(princess_row)) do 
      puts "Not a valid selection. Please try again"
      princess_row = gets.chomp.to_i
    end 
    ...
    until (range.include?(princess_column) && [princess_row, princess_column] != [bot_row, bot_column]) do 
      puts "Not a valid selection. Please try again"
      princess_column = gets.chomp.to_i
    end
    ...
    new_grid_object = Grid.new(size, bow_row, bot_column, finished_grid)
